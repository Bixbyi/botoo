...
@bot.command()
async def 더하기(ctx,amount,amount2):
    calcResult = int(amount) + int(amount2)
    await ctx.send("정답은 바로! : " + str(calcResult))
@bot.command()
async def 빼기(ctx,amount,amount2):
    calcResult = int(amount) - int(amount2)
    await ctx.send("정답은 : " + str(calcResult))
@bot.command()
async def 곱하기(ctx,amount,amount2):
    calcResult = int(amount) * int(amount2)
    await ctx.send("정답은 바로! 정답은 바로! : " + str(calcResult))
@bot.command()
async def 나누기 (ctx,amount,amount2) :
            try:
                calcResult = int(amount)/int(amount2)
                await ctx.send("정 답 은 바 로  ! : "+str(calcResult))
...
@bot.command()
async def 도와줘(ctx, content = None):
    channel = ctx.author
    if content == None:
        EMBED = discord.Embed(colour=505505,title='기초도움말')
        EMBED.add_field(name='일반 명령어',value='hb!도와줘 일반',inline=False)
        EMBED.add_field(name='디스코드 명령어', value='hb!도와줘 디코', inline=False)
        EMBED.add_field(name='검색 명령어', value='hb!도와줘 검색', inline=False)
        EMBED.add_field(name='숫자 명령어', value='hb!도와줘 숫자', inline=False)
        EMBED.add_field(name='번역 명령어', value='hb!도와줘 번역', inline=False)
        EMBED.add_field(name='질병 명령어', value='hb!코로나', inline=False)
        EMBED.add_field(name='놀이 명령어', value='hb!주사위', inline=False)
        EMBED.add_field(name='관리자 명령어', value='hb!도와줘 관리자', inline=False)
        await channel.send(embed=EMBED)
    if content == '일반':
        embad = discord.Embed(colour=505505,title='디퉤일한 도움말')
        embad.add_field(name='1번 목록',value='여담, 하이빅스비, 삼성, 얼굴, 시리, 시리야, 핑, ',inline=False)
        embad.add_field(name='2번 목록',value='빅스비, 파이썬, 레게노, 깡, 돈줘, 애교해줘,지건',inline=False)
